...
for i=1:size(X,1)
for j=1:size(X,2)
    idx=find(xTest(:,1)==0.30 & xTest(:,2)==0.45 & xTest(:,3)==X(i,j) & xTest(:,4)==Y(i,j));
    ySource(i,j)=mean(yTest_mean_cpu(idx,2));
    idx_xTestSource=[idx_xTestSource;idx];
end
end
...
for i=1:size(X,1)
for j=1:size(X,2)
    idx=find(xTest(:,1)==0 & xTest(:,2)==0 & xTest(:,3)==X(i,j) & xTest(:,4)==Y(i,j));
    yTarget(i,j)=mean(yTest_mean_cpu(idx,2));
    idx_xTestTarget=[idx_xTestTarget;idx];
end
end
...
for i=1:25
    for j=1:27
        if ~isnan(ySource(j,i)) && ~isnan(yTarget(j,i))
...
for i=1:length(idx_xTrainSource)
idx=find(xTest2(:,1)==X(idx_xTrainSource(i,2),idx_xTrainSource(i,1)) & xTest2(:,2)==Y(idx_xTrainSource(i,2),idx_xTrainSource(i,1)));
if ~isempty(idx)
idx_target(i,1)=find(xTest2(:,1)==X(idx_xTrainSource(i,2),idx_xTrainSource(i,1)) & xTest2(:,2)==Y(idx_xTrainSource(i,2),idx_xTrainSource(i,1)));
end
end
xTest2=xTest2(idx_target,:);
...
idx_target=[];
for i=1:length(xTrainTarget)
idx=find(xTest2(:,1)==X(xTrainTarget(i,2),xTrainTarget(i,1)) & xTest2(:,2)==Y(xTrainTarget(i,2),xTrainTarget(i,1)));
if ~isempty(idx)
...
for i=1:length(xTrainSource)
    idx=find(xTest2(:,1)==X(xTrainSource(i,2),xTrainSource(i,1)) & xTest2(:,2)==Y(xTrainSource(i,2),xTrainSource(i,1)));
    if ~isempty(idx)
        idx_source(i,1)=idx;
    end
end
