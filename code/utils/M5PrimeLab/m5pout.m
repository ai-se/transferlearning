...
%   precision     : Number of digits in the regression model coefficients,
%                   split values etc. (default value = 15)
...
%                   child to 'false'. This argument is ignored if model
%                   contains decision rules.
%   plotFontSize  : Font size for text in the plot (default value = 10).
%                   This argument is ignored if plotTree = false or model
%                   contains decision rules.
    ...
    if isempty(rules)
        ...
        for j = 1 : length(rules)
            ...
            if model.binCat.binCatNew(rule.attr) % a binary variable (might be synthetic)
                ...
                if rule.le
                    str = [str zx num2str(rule.attr) ' <= ' num2str(rule.location) ' '];
                else
                    str = [str zx num2str(rule.attr) ' > ' num2str(rule.location) ' '];
                end
            end
