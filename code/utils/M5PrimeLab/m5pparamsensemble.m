function trainParamsEnsemble = m5pparamsensemble(numTrees, numVarsTry, ...
    ...
    getVarImportance, verboseNumIter)
% m5pparamsensemble
...
% also accepted (the corresponding default values will be used). The first
% five arguments control the behaviour of the ensemble building method. The
% last three arguments enable getting additional information.
...
%                   directly from supplied training data set, before
%                   synthetic binary variables are made, if any.
...
%                   stable estimate, but the process is even slower.
...
%                   values (or default ones, if not provided).
%
% Remarks:
...
if (nargin < 8) || isempty(verboseNumIter)
